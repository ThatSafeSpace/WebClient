...
import { faTimes, faThumbsDown, faThumbsUp, faStarHalfAlt } from '@fortawesome/free-solid-svg-icons';
...
export class ListComponent implements OnInit {
  constructor(private vps: ViewportScroller, public apiService: ApiService, public globals: Globals, public menuService: MenuService, private spinner: NgxSpinnerService) { }
  //variables
  displayResults = [];
  ...
  showFiveMoreEntries(){
    this.menuService.object.filterNumberValue = (+this.menuService.object.filterNumberValue) + (+5);
  }
    ...
    if (this.globals.all_entries.length == 0) {
      ...
      this.apiService.getAllEntries().subscribe(arg => {
        ...
        if (this.globals.currentID){
          var recentEntry = this.globals.all_entries.find(e => e.id === this.globals.currentID)
          var recentEntryPlacement = this.globals.all_entries.indexOf(recentEntry)
